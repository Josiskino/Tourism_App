...
import 'package:myapp/features/home_page/presentation/pages/category_select_screen/category_select.dart';
...
class _NewHomeState extends State<NewHome> {
  ...
  void initState() {
    super.initState();
    context.read<HomePageBloc>().add(const LoadHomePageDataEvent());
  }
      ...
      child: Scaffold(
        ...
        body: BlocBuilder<HomePageBloc, HomePageState>(
          builder: (context, state) {
            // Handle different states of the bloc
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        context
                            .read<HomePageBloc>()
                            .add(const LoadHomePageDataEvent());
                      },
            ...
            // Convertir les catégories du state en PlaceCategory
            final categories = state.categories
                .map((category) => PlaceCategory(
                      icon: _mapCategoryToIcon(category.name),
                      name: category.name,
                    ))
                .toList();
            ...
            return SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  _buildCategoriesList(categories: categories),
                  const SizedBox(height: sectionSpace),
    ...
    return Padding(
      ...
      child: Column(
        children: [
          CarouselSlider(
            items: _imagesAsset
                .map((assetPath) => _buildCarouselItem(assetPath))
                .toList(),
    ...
    return Row(
      ...
      children: _imageUrls.asMap().entries.map((entry) {
        ...
        return GestureDetector(
          ...
          child: Container(
            width: isSelected ? 16.0 : 7.0,
            height: isSelected ? 4.0 : 7.0,
  ...
  Widget _buildCategoriesList({List<PlaceCategory> categories = const []}) {
    return SizedBox(
  ...
  Widget _buildNearbyPlacesList() {
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        scrollDirection: Axis.horizontal,
        padding: const EdgeInsets.symmetric(horizontal: appPadding),
        itemCount: state.recentSites.length,
        separatorBuilder: (_, __) => const SizedBox(width: 13),
  ...
  Widget _buildPlaceImage(int index) {
    // Récupérer le state actuel
    final state = context.read<HomePageBloc>().state;
    final site = state.recentSites[index];

    final mainPhoto = site.photos.firstWhere(
      (photo) => photo
          .isMain, /*orElse: () => site.photos.isNotEmpty ? site.photos.first : null*/
    );
          ...
          onTap: () => Navigator.push(
            ...
            PageRouteBuilder(
              ...
              pageBuilder: (context, animation, secondaryAnimation) {
                return FadeTransition(
                  opacity: animation,
                  child: PlaceDetailsScreen(site: site),
                );
              },
              transitionsBuilder:
                  (context, animation, secondaryAnimation, child) {
              ...
              Hero(
                ...
                child: ClipRRect(
                  ...
                  child: mainPhoto != null
                      ...
                      : Image.asset(
                          'assets/placeholder_image.png', // Ajoutez une image de placeholder
                          height: 200,
                          width: 190,
                          fit: BoxFit.cover,
                        ),
...
// }
  ...
  Widget _buildPlaceInfo(int index) {
    // Récupérer le state actuel
    final state = context.read<HomePageBloc>().state;
    final site = state.recentSites[index];
